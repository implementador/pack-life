            ...
            for attachment_name in z.namelist():
                with z.open(attachment_name) as att_xml:
                    ...
                    if cfdi_type in ['SP', 'P']:
                        ...
                        pagos = complemento.find('pago20:Pagos', NSMAP) if cfdi_version == '4.0' else complemento.find(
                            'pago10:Pagos', NSMAP)
                        pago = pagos.find('pago20:Pago', NSMAP) if cfdi_version == '4.0' else pagos.find('pago10:Pago',
                                                                                                         NSMAP)
                        ...
                        if cfdi_type == 'SP':
                            for uu in [uuid, uuid.lower(), uuid.upper()]:
                                payment_exist = payment_obj.search(
                                    [('l10n_mx_edi_cfdi_uuid_cusom', '=', uu), ('payment_type', '=', 'outbound')], limit=1)
                                if payment_exist:
                                    vals.update({'creado_en_odoo': True, 'payment_ids': [(6, 0, payment_exist.ids)]})
                                    break
                        elif cfdi_type == 'SE':
                            for uu in [uuid, uuid.lower(), uuid.upper()]:
                                invoice_exist = invoice_obj.search(
                                    [('l10n_mx_edi_cfdi_uuid_cusom', '=', uu), ('move_type', '=', 'in_refund')],
                                    limit=1)
                                if invoice_exist:
                                    vals.update({'creado_en_odoo': True, 'invoice_ids': [(6, 0, invoice_exist.ids)]})
                                    break
                            ...
                            for uu in [uuid, uuid.lower(), uuid.upper()]:
                                invoice_exist = invoice_obj.search(
                                    [('l10n_mx_edi_cfdi_uuid_cusom', '=', uu), ('move_type', '=', 'in_invoice')], limit=1)
                                if invoice_exist:
                                    vals.update({'creado_en_odoo': True, 'invoice_ids': [(6, 0, invoice_exist.ids)]})
                                    break
    ...
    def download_cfdi_invoices_web(self, start_date=False, end_Date=False):
        ...
        if invoice_content_receptor:
            ...
            for uuid,data in invoice_content_receptor.items():
                ...
                if cfdi_type=='SE':
                    for uu in [uuid,uuid.lower(),uuid.upper()]:
                        invoice_exist = invoice_obj.search([('l10n_mx_edi_cfdi_uuid_cusom','=',uu),('move_type','=','in_refund')],limit=1)
                        if invoice_exist:
                            vals.update({'creado_en_odoo' : True,'payment_ids':[(6,0, invoice_exist.ids)]})
                            break
                else:
                    for uu in [uuid,uuid.lower(),uuid.upper()]:
                        invoice_exist = invoice_obj.search([('l10n_mx_edi_cfdi_uuid_cusom','=',uu),('move_type','=','in_invoice')],limit=1)
                        if invoice_exist:
                            vals.update({'creado_en_odoo' : True,'invoice_ids':[(6,0, invoice_exist.ids)]})
                            break
                attachment_obj.create(vals)
        ...
        if invoice_content_emisor:
            ...
            for uuid,data in invoice_content_emisor.items():
                ...
                if cfdi_type=='E':
                    for uu in [uuid,uuid.lower(),uuid.upper()]:
                        invoice_exist = invoice_obj.search([('l10n_mx_edi_cfdi_uuid_cusom','=',uu),('move_type','=','out_refund')],limit=1)
                        if invoice_exist:
                            vals.update({'creado_en_odoo' : True,'payment_ids':[(6,0, invoice_exist.ids)]})
                            break
                else:
                    for uu in [uuid,uuid.lower(),uuid.upper()]:
                        invoice_exist = invoice_obj.search([('l10n_mx_edi_cfdi_uuid_cusom','=',uu),('move_type','=','out_invoice')],limit=1)
                        if invoice_exist:
                            vals.update({'creado_en_odoo' : True,'invoice_ids':[(6,0, invoice_exist.ids)]})
                            break
