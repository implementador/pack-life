    ...
    def import_xml_file(self):
        ...
        if model=='ir.attachment' and active_ids:
            ...
            for attachment in attachments:
                ...
                if ext[1:].lower() !='xml':
                    not_imported_attachment.update({attachment.name:_("Formato no soportado \"{}\", importa solo archivos XML").format(attachment.name)})
                    continue
                ...
                try:
                    ...
                    if res and type(res)==dict:
                        ...
                        if res.get('res_model')=='account.move':
                            val.update({'invoice_ids':[(6,0, [res.get('res_id')])], 'res_id': res.get('res_id'), 'res_model': 'account.move'})
