...
var MultiattachXmlsWizard = FieldChar.extend({
    template: 'multi_attach_xmls_template',
    ...
    events: {
        ...
        'click #dndfooter button#save': function(e){
            ...
            if(Object.keys(this.files).length <= 0){
                this.displayNotification({
                    type: 'warning',
                    title: _t('Warning'),
                    message: "There is no files selected...",
                    sticky: true
                });
                ...
                this.displayNotification({
                    type: 'warning',
                    title: _t('Warning'),
                    message: "You cannot attach more than one xml to an invoice...",
                    sticky: true
                });
            }else{
        ...
        // Shows you the attachments created in a treeview
    ...
    sendFileToServer: function(files){
        /* Sends each base64 file string to the back-end server to create the attachments */
        var self = this;
    ...
    handleFileWrong: function(wrongfiles){
        /* Saves the exceptions occurred in the attachments creation */
    ...
    correctFinalRegistry: function(){
        /* Shows the final success alert and the button to see the attachments created */
        var self = this;
        var alert = $('<div class="alert alert-success dnd-alert"><strong>' + _t('Congratulations') + '!</strong> ' +
            _t('Your Attachments were created correctly') + '.</div>').hide();
        self.$el.find("#alertscontent").html(alert);
...
FieldRegistry.add('multi_attach_xmls_wizard_widget', MultiattachXmlsWizard);
return {MultiattachXmlsWizard: MultiattachXmlsWizard};
});
